...
import WeatherBackground from './components/WeatherBackgroundB';
...
const API_KEY='191a0d690fce1ad25b1916aabf911ad5'
...
//fetch 5 location suggestion from api and update it
...
const fetchSuggestion=async(query)=>{
  ...
  //this function prevent form submission validates city and fetches data via api
  const handleSearch=async (e)=>{
    e.preventDefault();
    if(!city.trim()) return setError ("please enter a valid city name")
      await fetchWeatherData(`http://api.openweathermap.org/data/2.5/weather?q=${city.trim()}&appid=${API_KEY}&units=metric`)
  }
            ...
            {suggestion.length>0 &&(
              <div className='absolute top-12 left-0 right-0 bg-transparent shadow-md rounded z-10'>
                {suggestion.map((s)=>(
                  ...
                  onClick={()=>fetchWeatherData(`http://api.openweathermap.org/data/2.5/weather?lat=${s.lat}&lon=${s.lon}&appid=${API_KEY}&units=metric`,`${s.name},${s.country}${s.state ?`,${s.state}`:``}`
